...
# A fixture é uma lista de eventos; pegamos o 'd' do primeiro GUILD_CREATE
my $guild-payload = $raw-json[0]<d>;
my $json-string = to-json($guild-payload);

diag "Real Fixture: gateway_guild_create.json (extracted 'd')";
diag "Payload Size: {($json-string.encode.elems / 1024).Rat.round(0.1)} KB";

my $parser = Dusk::Internal::Parser.new(mode => ModeRaku);

subtest 'Baseline Accuracy' => {
    my $guild = $parser.parse($json-string, model => Dusk::Model::Guild);
    ok $guild ~~ Dusk::Model::Guild, "Parsed into Guild object";
    ok $guild.name.defined, "Parsed name: {$guild.name}";
    diag "Members count: {$guild.members.elems if $guild.members}";
};
