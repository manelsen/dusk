...
subtest 'Endpoint: DELETE /applications/{application.id}/commands/{command.id}' => {
    ...
    if $route {
        ...
        is $route.path, '/applications/{application.id}/commands/{command.id}', 'Path structure is mostly correct';
    } else {
...
subtest 'Endpoint: DELETE /applications/{application.id}/emojis/{emoji.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/applications/{application.id}/emojis/{emoji.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /applications/{application.id}/entitlements/{entitlement.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/applications/{application.id}/entitlements/{entitlement.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /applications/{application.id}/guilds/{guild.id}/commands/{command.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/applications/{application.id}/guilds/{guild.id}/commands/{command.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/messages/pins/{message.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/messages/pins/{message.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/messages/{message.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/messages/{message.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/messages/{message.id}/reactions' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/messages/{message.id}/reactions', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/messages/{message.id}/reactions/{emoji.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.delete-channels-messages-reactions-2(channel-id => 'test-val', message-id => 'test-val', emoji-id => 'test-val') }, 'Method delete-channels-messages-reactions-2 exists and accepts parameters';
    
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/messages/{message.id}/reactions/{emoji.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/messages/{message.id}/reactions/{emoji.id}/@me' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/messages/{message.id}/reactions/{emoji.id}/@me', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/messages/{message.id}/reactions/{emoji.id}/{user.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.delete-channels-messages-reactions-3(channel-id => 'test-val', message-id => 'test-val', emoji-id => 'test-val', user-id => 'test-val') }, 'Method delete-channels-messages-reactions-3 exists and accepts parameters';
    
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/messages/{message.id}/reactions/{emoji.id}/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/permissions/{overwrite.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/permissions/{overwrite.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/pins/{message.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/pins/{message.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/recipients/{user.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/recipients/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/thread-members/@me' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/thread-members/@me', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /channels/{channel.id}/thread-members/{user.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/channels/{channel.id}/thread-members/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/auto-moderation/rules/{auto_moderation_rule.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/auto-moderation/rules/{auto_moderation_rule.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/bans/{user.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/bans/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/emojis/{emoji.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/emojis/{emoji.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/integrations/{integration.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/integrations/{integration.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/members/{user.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/members/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/members/{user.id}/roles/{role.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/members/{user.id}/roles/{role.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/roles/{role.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/roles/{role.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/scheduled-events/{guild_scheduled_event.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/scheduled-events/{guild_scheduled_event.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/soundboard-sounds/{sound.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/soundboard-sounds/{sound.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/stickers/{sticker.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/stickers/{sticker.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /guilds/{guild.id}/templates/{template.code}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/guilds/{guild.id}/templates/{template.code}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /invites/{invite.code}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/invites/{invite.code}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /lobbies/{lobby.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/lobbies/{lobby.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /lobbies/{lobby.id}/members/@me' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/lobbies/{lobby.id}/members/@me', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /lobbies/{lobby.id}/members/{user.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/lobbies/{lobby.id}/members/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /stage-instances/{channel.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/stage-instances/{channel.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /users/@me/guilds/{guild.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/users/@me/guilds/{guild.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /webhooks/{application.id}/{interaction.token}/messages/@original' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/webhooks/{application.id}/{interaction.token}/messages/@original', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /webhooks/{application.id}/{interaction.token}/messages/{message.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/webhooks/{application.id}/{interaction.token}/messages/{message.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /webhooks/{webhook.id}' => {
    ...
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/webhooks/{webhook.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /webhooks/{webhook.id}/{webhook.token}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.delete-webhooks-2(webhook-id => 'test-val', webhook-token => 'test-val') }, 'Method delete-webhooks-2 exists and accepts parameters';
    
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/webhooks/{webhook.id}/{webhook.token}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: DELETE /webhooks/{webhook.id}/{webhook.token}/messages/{message.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.delete-webhooks-messages-2(webhook-id => 'test-val', webhook-token => 'test-val', message-id => 'test-val') }, 'Method delete-webhooks-messages-2 exists and accepts parameters';
    
    if $route {
        is $route.method, 'DELETE', 'Method is DELETE';
        is $route.path, '/webhooks/{webhook.id}/{webhook.token}/messages/{message.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/activity-instances/{instance_id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/activity-instances/{instance_id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/commands' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/commands', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/emojis' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/emojis', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/entitlements' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/entitlements', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/guilds/{guild.id}/commands' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/guilds/{guild.id}/commands', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/guilds/{guild.id}/commands/permissions' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/guilds/{guild.id}/commands/permissions', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/guilds/{guild.id}/commands/{command.id}/permissions' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.get-applications-guilds-commands-permissions-2(application-id => 'test-val', guild-id => 'test-val', command-id => 'test-val') }, 'Method get-applications-guilds-commands-permissions-2 exists and accepts parameters';
    
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/guilds/{guild.id}/commands/{command.id}/permissions', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/role-connections/metadata' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/role-connections/metadata', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /applications/{application.id}/skus' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/{application.id}/skus', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/invites' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/invites', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/messages' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/messages', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/messages/pins' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/messages/pins', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/pins' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/pins', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/polls/{message.id}/answers/{answer_id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/polls/{message.id}/answers/{answer_id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/thread-members' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/thread-members', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/threads/archived/private' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/threads/archived/private', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/threads/archived/public' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/threads/archived/public', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/users/@me/threads/archived/private' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/users/@me/threads/archived/private', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /channels/{channel.id}/webhooks' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/channels/{channel.id}/webhooks', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/templates/{template.code}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/templates/{template.code}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/audit-logs' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/audit-logs', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/auto-moderation/rules' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/auto-moderation/rules', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/bans' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/bans', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/channels' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/channels', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/emojis' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/emojis', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/integrations' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/integrations', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/invites' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/invites', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/members' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/members', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/members/search' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/members/search', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/onboarding' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/onboarding', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/preview' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/preview', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/prune' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/prune', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/regions' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/regions', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/roles' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/roles', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/roles/member-counts' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/roles/member-counts', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/scheduled-events' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/scheduled-events', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/scheduled-events/{guild_scheduled_event.id}/users' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/scheduled-events/{guild_scheduled_event.id}/users', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/soundboard-sounds' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/soundboard-sounds', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/stickers' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/stickers', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/templates' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/templates', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/threads/active' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/threads/active', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/vanity-url' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/vanity-url', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/voice-states/@me' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/voice-states/@me', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/voice-states/{user.id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/voice-states/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/webhooks' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/webhooks', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/welcome-screen' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/welcome-screen', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/widget' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/widget', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/widget.json' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.get-guilds-widget.json(guild-id => 'test-val') }, 'Method get-guilds-widget.json exists and accepts parameters';
    
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/widget.json', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /guilds/{guild.id}/widget.png' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.get-guilds-widget.png(guild-id => 'test-val') }, 'Method get-guilds-widget.png exists and accepts parameters';
    
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/guilds/{guild.id}/widget.png', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /invites/{invite.code}/target-users' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/invites/{invite.code}/target-users', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /invites/{invite.code}/target-users/job-status' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/invites/{invite.code}/target-users/job-status', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /skus/{sku.id}/subscriptions' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/skus/{sku.id}/subscriptions', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /skus/{sku.id}/subscriptions/{subscription.id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/skus/{sku.id}/subscriptions/{subscription.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /sticker-packs/{pack.id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/sticker-packs/{pack.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /stickers/{sticker.id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/stickers/{sticker.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /users/@me/applications/{application.id}/role-connection' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/users/@me/applications/{application.id}/role-connection', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /users/@me/guilds/{guild.id}/member' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/users/@me/guilds/{guild.id}/member', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /users/{user.id}' => {
    ...
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/users/{user.id}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: GET /webhooks/{webhook.id}/{webhook.token}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.get-webhook-2(webhook-id => 'test-val', webhook-token => 'test-val') }, 'Method get-webhook-2 exists and accepts parameters';
...
subtest 'Endpoint: GET /webhooks/{webhook.id}/{webhook.token}/messages/{message.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.get-webhooks-message-2(webhook-id => 'test-val', webhook-token => 'test-val', message-id => 'test-val') }, 'Method get-webhooks-message-2 exists and accepts parameters';
...
subtest 'Endpoint: PATCH /guilds/{guild.id}/members/@me' => {
    ...
    if $route {
        is $route.method, 'PATCH', 'Method is PATCH';
        is $route.path, '/guilds/{guild.id}/members/@me', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: PATCH /guilds/{guild.id}/members/@me/nick' => {
    ...
    if $route {
        is $route.method, 'PATCH', 'Method is PATCH';
        is $route.path, '/guilds/{guild.id}/members/@me/nick', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: PATCH /guilds/{guild.id}/roles/{role.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.patch-guilds-roles-2(guild-id => 'test-val', role-id => 'test-val') }, 'Method patch-guilds-roles-2 exists and accepts parameters';
...
subtest 'Endpoint: PATCH /lobbies/{lobby.id}/channel-linking' => {
    ...
    if $route {
        is $route.method, 'PATCH', 'Method is PATCH';
        is $route.path, '/lobbies/{lobby.id}/channel-linking', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: PATCH /webhooks/{webhook.id}/{webhook.token}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.patch-webhooks-2(webhook-id => 'test-val', webhook-token => 'test-val') }, 'Method patch-webhooks-2 exists and accepts parameters';
...
subtest 'Endpoint: PATCH /webhooks/{webhook.id}/{webhook.token}/messages/{message.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.patch-webhooks-messages-2(webhook-id => 'test-val', webhook-token => 'test-val', message-id => 'test-val') }, 'Method patch-webhooks-messages-2 exists and accepts parameters';
...
subtest 'Endpoint: POST /applications/{application.id}/entitlements/{entitlement.id}/consume' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/applications/{application.id}/entitlements/{entitlement.id}/consume', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/followers' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/followers', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/messages/bulk-delete' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/messages/bulk-delete', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/messages/{message.id}/crosspost' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/messages/{message.id}/crosspost', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/messages/{message.id}/threads' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/messages/{message.id}/threads', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/polls/{message.id}/expire' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/polls/{message.id}/expire', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/send-soundboard-sound' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/send-soundboard-sound', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/threads' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/threads', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /channels/{channel.id}/typing' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/channels/{channel.id}/typing', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /guilds/{guild.id}/bulk-ban' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/guilds/{guild.id}/bulk-ban', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /interactions/{interaction.id}/{interaction.token}/callback' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/interactions/{interaction.id}/{interaction.token}/callback', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /webhooks/{application.id}/{interaction.token}' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/webhooks/{application.id}/{interaction.token}', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /webhooks/{webhook.id}/{webhook.token}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.post-webhooks-2(webhook-id => 'test-val', webhook-token => 'test-val') }, 'Method post-webhooks-2 exists and accepts parameters';
...
subtest 'Endpoint: POST /webhooks/{webhook.id}/{webhook.token}/github' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/webhooks/{webhook.id}/{webhook.token}/github', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: POST /webhooks/{webhook.id}/{webhook.token}/slack' => {
    ...
    if $route {
        is $route.method, 'POST', 'Method is POST';
        is $route.path, '/webhooks/{webhook.id}/{webhook.token}/slack', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: PUT /applications/{application.id}/guilds/{guild.id}/commands/{command.id}/permissions' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.put-applications-guilds-commands-permissions-2(application-id => 'test-val', guild-id => 'test-val', command-id => 'test-val') }, 'Method put-applications-guilds-commands-permissions-2 exists and accepts parameters';
...
subtest 'Endpoint: PUT /guilds/{guild.id}/incident-actions' => {
    ...
    if $route {
        is $route.method, 'PUT', 'Method is PUT';
        is $route.path, '/guilds/{guild.id}/incident-actions', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};
...
subtest 'Endpoint: PATCH /guilds/{guild.id}/onboarding' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.patch-guilds-onboarding(guild-id => 'test-val') }, 'Method patch-guilds-onboarding exists and accepts parameters';
...
subtest 'Endpoint: PATCH /guilds/{guild.id}/incident-actions' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.patch-guilds-incident-actions(guild-id => 'test-val') }, 'Method patch-guilds-incident-actions exists and accepts parameters';
...
subtest 'Endpoint: GET /applications/{application.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.get-application(application-id => 'test-val') }, 'Method get-application exists and accepts parameters';
    
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/test-val', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};

subtest 'Endpoint: PATCH /applications/{application.id}' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.patch-application(application-id => 'test-val') }, 'Method patch-application exists and accepts parameters';
...
subtest 'Endpoint: GET /applications/{application.id}/role-connections' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.get-applications-role-connections(application-id => 'test-val') }, 'Method get-applications-role-connections exists and accepts parameters';
    
    if $route {
        is $route.method, 'GET', 'Method is GET';
        is $route.path, '/applications/test-val/role-connections', 'Path structure is mostly correct';
    } else {
        skip 'Route generation failed, skipping assertions', 2;
    }
};

subtest 'Endpoint: PUT /applications/{application.id}/role-connections' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.put-applications-role-connections(application-id => 'test-val') }, 'Method put-applications-role-connections exists and accepts parameters';
...
subtest 'Endpoint: POST /applications/{application.id}/entitlements' => {
    my $route;
    lives-ok { $route = Dusk::Rest::Endpoint.post-applications-entitlements-create(application-id => 'test-val') }, 'Method post-applications-entitlements-create exists and accepts parameters';
